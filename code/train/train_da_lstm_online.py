...
class Trainer:
    ...
    def predict(self, x, y, y_seq):
        ...
        return y_pred
    """
    def save_train(self, x, y, y_seq):
        #x_test, y_test, y_seq_test = self.dataset.get_test_set()
        y_pred_train = self.predict(x, y, y_seq)
        current_train_class = [1 if ele>thresh else 0 for ele in y_pred_train]
        current_test_class = [1 if ele>thresh else 0 for ele in y]
        #print("the  accuracy: ",accuracy_score(current_test_class,y_pred_test))
        return current_train_class
